#ifndef L1Trigger_L1TCaloLayer1_UCTObject_hh
#define L1Trigger_L1TCaloLayer1_UCTObject_hh
...
public:
  ...
  //const uint32_t nEGammas() const { return myNEGammas; }
  ...
  const std::vector<uint32_t> boostedJetRegionEGammaVeto() const { return myBoostedJetRegionEGammaVeto; }
  bool setNTaus(uint32_t in) {
    myNTaus = in;
    return true;
  }
  //bool setNEGammas(uint32_t in) {
  //  myNEGammas = in;
  //  return true;
  //}
  ...
  bool setBoostedJetRegionEGammaVeto(std::vector<uint32_t> in) {
    myBoostedJetRegionEGammaVeto = in;
    return true;
  }
...
private:
  ...
  std::vector<uint32_t> myBoostedJetRegionEGammaVeto;
};
