...
private:
  ...
  //edm::ESGetToken<CaloTPGTranscoder> caloTPGToken_;
  ...
  //edm::ESGetToken<HcalTopology> hbTopoToken_;
...
Phase2L1CaloEGammaEmulator::Phase2L1CaloEGammaEmulator(const edm::ParameterSet & iConfig)
  : ecalTPEBToken_(consumes<EcalEBTrigPrimDigiCollection>(iConfig.getParameter<edm::InputTag>("ecalTPEB"))),
    ...
    hbTopologyToken_(esConsumes<HcalTopology, HcalRecNumberingRecord>()){
...
void Phase2L1CaloEGammaEmulator::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  //edm::ESHandle<HcalTopology> hbTopology;
  //iSetup.getByToken(hbTopoToken_, hbTopology);
  //iSetup.get<HcalRecNumberingRecord>().get(hbTopology);
  //edm::ESGetToken<hbTopoToken_, hbTopology>
  ////hbTopologyToken_ = esConsumes<HcalTopology, HcalRecNumberingRecord>();
  //hcTopology_ = hbTopology.product();
  hcTopology_ = &iSetup.getData(hbTopologyToken_);
  HcalTrigTowerGeometry theTrigTowerGeometry(hcTopology_);
  ////decoderToken_ = esConsumes<CaloTPGTranscoder, CaloTPGRecord>();
  //edm::ESHandle<CaloTPGTranscoder> decoder_ = iSetup.getData(decoderToken_);
  decoder_ = &iSetup.getData(decoderToken_);
  //edm::ESHandle<CaloTPGTranscoder> decoder_;
  //iSetup.getByToken(caloTPGToken_, decoder_);
